...
print(os.listdir("/root/.local/lib/python3.5/site-packages/trainer/"))
...
def main(unused_argv):
  ...
  tf.compat.v1.logging.info('Precision: %s', params.precision)
  use_bfloat16 = params.precision == 'bfloat16'

  # Input pipelines are slightly different (with regards to shuffling and
  # preprocessing) between training and evaluation.
  if FLAGS.bigtable_instance:
    tf.compat.v1.logging.info('Using Bigtable dataset, table %s', FLAGS.bigtable_table)
    ...
    if FLAGS.data_dir == FAKE_DATA_DIR:
      ...
      tf.compat.v1.logging.info('Using dataset: %s', FLAGS.data_dir)
  ...
  if FLAGS.mode == 'eval':
    ...
    for ckpt in evaluation.checkpoints_iterator(
      ...
      try:
        ...
        tf.compat.v1.logging.info('Eval results: %s. Elapsed seconds: %d',
    ...
    steps_per_epoch = params.num_train_images // params.train_batch_size
    tf.compat.v1.logging.info('Training for %d steps (%.2f epochs in total). Current'
                    ' step %d.',
                    params.train_steps,
    ...
    if FLAGS.mode == 'train':
      ...
      while current_step < params.train_steps:
        ...
        tf.compat.v1.logging.info('Finished training up to step %d. Elapsed seconds %d.',
        ...
        tf.compat.v1.logging.info('Eval results at step %d: %s',
